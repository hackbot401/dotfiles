...
import qualified XMonad.Layout.ToggleLayouts as T (toggleLayouts, ToggleLayout(Toggle))       -- Full window at any time
...
myWorkspaces = ["MAIN", "WWW", "MUSIC", "EDIT", "FILES", "VIDEO", "FLT", "VIRT", "WATCH"]
...
projects =
    ...
    [ Project   { projectName       = wsVIRT
                , projectDirectory  = "~/"
                , projectStartHook  = Just $ do spawnOn wsVIRT "virt-manager"
                }

    , Project   { projectName       = wsEDIT
                , projectDirectory  = "~/"
                , projectStartHook  = Just $ do spawnOn wsEDIT myEditor
                }
...
myEditor            = "emacs"
myStatusBar         = "xmobar -x0 /home/hackbot/.xmonad/xmobar.conf"
myLauncher          = "dmenu_run -nb '#2f343f' -fn 'Fira Code Nerd Font:size=11' -nf '#e7e8eb' -sb '#93d44f' -sf '#2f343f'"
-- myLauncher          = "dmenu_run -nb '#000' -nf '#FFF' -fn '-xos4-terminus-bold-r-normal--14-140-72-72-*-*-iso10646-1' "
...
hiangoutsTitle     = "Google Hangouts - es@ethanschoonover.com"
...
--nuclearCommand      = "nuclear"
--nuclearInfix        = "nuclear"
--nuclearResource     = "crx_ioljlgoncmlkbcepmminebblkddfjofl"
--isnuclear           = (resource =? "nuclear")

--plexCommand         = "plexmediaplayer"
--plexInfix           = "plexmediaplayer"
--plexResource        = "crx_fpniocchabmgenibceglhnfeimmdhdfm"
--isPlex              = (resource =? "plexmediaplayer")

isConsole           = (className =? "kitty")
                    <&&> (stringProperty "WM_WINDOW_ROLE" =? "Scratchpad")
myConsole           = "kitty -T console -p console --role=Scratchpad"
...
scratchpads =
    ...
    ,   (NS "nuclear" "nuclear" (resource =? "nuclear") (customFloating $ W.RationalRect (2/3) (2/6) (2/5) (2/3)) )
    ,   (NS "plexmediaplayer" "plexmediaplayer" (resource =? "plexmediaplayer") (customFloating $ W.RationalRect (2/3) (2/6) (2/5) (2/3)) )
...
myFont      = "-*-Roboto Mono-medium-*-*-*-*-100-*-*-*-*-*-*"
myBigFont   = "-*-Envy Code R-medium-*-*-*-*-240-*-*-*-*-*-*"
...
topBarTheme = def
    ...
    , inactiveTextColor     = cyan
    , activeBorderColor     = violet
    , activeColor           = violet
    , activeTextColor       = violet
    , urgentBorderColor     = violet
...
showKeybindings x = addName "Show Keybindings" $ io $ do
    h <- spawnPipe "zenity --text-info --font=Envy Code R"
...
myKeys conf = let
  ...
  --, ("M-7"                    , addName "TESTING"                         $ runInTerm "-name glances" "glances" )
    ...
    , ("M-c"                    , addName "Record screen - area select"     $ spawn "$HOME/colorschemes-changer" )
    ] ^++^
    ...
    , ("M-b"                    , addName "AltBrowser"                      $ spawn myAltBrowser)
    , ("M-e"                    , addName "Editor"                          $ spawn myEditor)
    , ("M-t"                    , addName "AltTerminal"                     $ spawn myAltTerminal)
    , ("M-m"                    , addName "NSP Music"                       $ namedScratchpadAction scratchpads "nuclear")
    , ("M-v"                    , addName "NSP Video"                       $ namedScratchpadAction scratchpads "plexmediaplayer")
    ...
    subKeys "Actions"
    [ ("M-p h"                    , addName "Notify w current X selection"    $ spawn "dm-hub")
    , ("M-p a"                    , addName "TESTING"                         $ spawn "dm-sounds")
    , ("M-p b"                    , addName "TESTING"                         $ spawn "dm-setbg")
    , ("M-p c"                    , addName "Copy current browser URL"        $ spawn "dtos-colorscheme")
    , ("M-p C"                    , addName "Display (output) launcher"       $ spawn "dm-colpick")
    , ("M-p e"                    , addName "Display - force internal"        $ spawn "dm-confedit")
    , ("M-p i"                    , addName "Display - force internal"        $ spawn "dm-maim")
    , ("M-p k"                    , addName "Network (Interface) launcher"    $ spawn "dm-kill")
    , ("M-p m"                    , addName "On-screen keys"                  $ spawn "dm-man")
    , ("M-p n"                    , addName "On-screen keys"                  $ spawn "dm-note")
    , ("M-p o"                    , addName "On-screen keys settings"         $ spawn "dm-bookman")
    , ("M-p p"                    , addName "Capture screen"                  $ spawn "passmenu" )
    , ("M-p q"                    , addName "Capture screen - area select"    $ spawn "dm-logout" )
    , ("M-p r"                    , addName "Record screen"                   $ spawn "dm-reddit" )
    , ("M-p s"                    , addName "Record screen - area select"     $ spawn "dm-websearch" )
    , ("M-p t"                    , addName "Record screen - area select"     $ spawn "dm-translate" )
    , ("M-p y"                    , addName "youtube-CLI"                     $ spawn "ytfzf -D" )
    ] ^++^
...
myStartupHook = do
    ...
    --spawn "/home/hackbot/bin/wm/init-wallpaper"
...
myManageHook =
    ...
    where
        manageSpecific = composeOne
            ...
            --, resource =? nuclearResource -?> doCenterFloat
